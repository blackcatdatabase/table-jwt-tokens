 ...
 */
final class JwtTokensAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private JwtTokenRepository $jwtTokenRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'jwt_tokens' => $this->jwtTokenRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\JwtTokens\ModuleException("Record {$idStr} not found for locking.");
            }

...
use BlackCat\Database\Packages\JwtTokens\Repository\JwtTokenRepositoryInterface;
...
class JwtTokenRepository implements JwtTokenRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'token_hash' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\JwtTokens\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'jti', 'user_id', 'token_hash_algo', 'token_hash_key_version', 'type', 'scopes', 'expires_at', 'last_used_at', 'ip_hash', 'ip_hash_key_version', 'replaced_by', 'revoked', 'meta' ]);
          return count($rows);
        }
    ...
    public function getByJti(string $jti, bool $asDto = false): array|\BlackCat\Database\Packages\JwtTokens\Dto\JwtTokenDto|null {
        return $this->getByUnique([ 'jti' => $jti ], $asDto);
    }

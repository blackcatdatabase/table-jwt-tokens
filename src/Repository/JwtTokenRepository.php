...
namespace BlackCat\Database\Packages\JwtTokens\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\JwtTokens\Definitions;
use BlackCat\Database\Packages\JwtTokens\Criteria;
use BlackCat\Database\Packages\JwtTokens\Dto\JwtTokenDto as Dto;
use BlackCat\Database\Packages\JwtTokens\Mapper\JwtTokenDtoMapper as RowMapper;
...
final class JwtTokenRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\JwtTokens\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $updCols = [ 'jti', 'user_id', 'token_hash_algo', 'token_hash_key_version', 'type', 'scopes', 'expires_at', 'last_used_at', 'ip_hash', 'ip_hash_key_version', 'replaced_by', 'revoked', 'meta' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'token_hash' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\JwtTokens\Definitions::table());
            return $bulk->upsertMany($rows, [ 'token_hash' ], [ 'jti', 'user_id', 'token_hash_algo', 'token_hash_key_version', 'type', 'scopes', 'expires_at', 'last_used_at', 'ip_hash', 'ip_hash_key_version', 'replaced_by', 'revoked', 'meta' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\JwtTokens\Dto\JwtTokenDto|null */
    public function getByTokenHash(string $tokenHash, bool $asDto = false): array|\BlackCat\Database\Packages\JwtTokens\Dto\JwtTokenDto|null {
        $row = $this->getByUnique([ 'token_hash' => $tokenHash ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\JwtTokens\Mapper\JwtTokenDtoMapper::fromRow($row);
    }
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\JwtTokens\Dto\JwtTokenDto|null */
    public function getByJti(string $jti, bool $asDto = false): array|\BlackCat\Database\Packages\JwtTokens\Dto\JwtTokenDto|null {
        $row = $this->getByUnique([ 'jti' => $jti ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\JwtTokens\Mapper\JwtTokenDtoMapper::fromRow($row);
    }
    public function existsByJti(string $jti): bool {
        $where = 't.' . Ident::q($this->db, 'jti') . ' = :uniq_jti';
        return $this->exists($where, [ 'uniq_jti' => $jti ]);
    }
    /** @return int|string|null */
    public function getIdByJti(string $jti) {
        $row = $this->getByJti($jti);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\JwtTokens\Dto\JwtTokenDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\JwtTokens\Dto\JwtTokenDto|null {
